...
int
request_vote_request_candidate_id_assign(struct request_vote_request *msg, const ev_uint64_t value)
{
  msg->candidate_id_set = 1;
  msg->candidate_id_data = value;
  return (0);
}
...
int
request_vote_request_candidate_id_get(struct request_vote_request *msg, ev_uint64_t *value)
{
...
request_vote_request_marshal(struct evbuffer *evbuf, const struct request_vote_request *tmp){
  ...
  evtag_marshal_int64(evbuf, REQUEST_VOTE_REQUEST_CANDIDATE_ID, tmp->candidate_id_data);
    ...
    switch (tag) {
      ...
      case REQUEST_VOTE_REQUEST_CANDIDATE_ID:
        ...
        if (tmp->candidate_id_set)
          return (-1);
        if (evtag_unmarshal_int64(evbuf, REQUEST_VOTE_REQUEST_CANDIDATE_ID, &tmp->candidate_id_data) == -1) {
